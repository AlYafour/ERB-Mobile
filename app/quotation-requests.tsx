...
function QuotationRequestsScreenInner() {
  ...
  const loadRequests = async () => {
    try {
      setError(null);
      setLoading(true);
      setData(await quotationRequestsApi.getAll({ page, page_size: 50, search: debouncedSearch, ...filters }));
    } catch (err: any) {
      ...
      <AppCard style={S.itemCard} onPress={() => router.push(`/quotation-requests/${itemId}` as any)}>
        ...
        {prNumber ? (
          <View style={S.metaRow}>
            <Text style={[S.metaLabel, { color: C.textMuted }]}>Purchase Request</Text>
            <Text style={[S.metaValue, { color: C.textPrimary }]} numberOfLines={1}>{prNumber}</Text>
          </View>
        ) : null}
        {supplierCount !== null ? (
          <View style={S.metaRow}>
            <Text style={[S.metaLabel, { color: C.textMuted }]}>Suppliers</Text>
            <Text style={[S.metaValue, { color: C.textSecondary }]}>
        ...
        {createdDate ? (
          <View style={S.metaRow}>
            <Text style={[S.metaLabel, { color: C.textMuted }]}>Created</Text>
            <Text style={[S.metaValue, { color: C.textSecondary }]}>{createdDate}</Text>
          </View>
        ) : null}
      </AppCard>
    );
      ...
      <View style={S.inner}>
        ...
        <View style={S.searchContainer}>
          <View style={S.searchRow}>
            <View style={S.searchInputWrapper}>
              <Input placeholder="Search quotation requests..." value={search} onChangeText={setSearch}
        ...
        {loading && !refreshing ? (
          ...
          <AppEmptyState variant="error" title="Failed to load" message={error}
            actionLabel="Try Again" onAction={loadRequests} />
        ) : !data?.results?.length ? (
